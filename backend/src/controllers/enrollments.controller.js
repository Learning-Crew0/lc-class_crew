...
const markAttendance = async (req, res, next) => {
  try {
    ...
    return successResponse(res, enrollment, "Attendance marked successfully");
  } catch (error) {
    next(error);
  }
};
...
const addAssessment = async (req, res, next) => {
  try {
    ...
    return successResponse(res, enrollment, "Assessment added successfully");
  } catch (error) {
    next(error);
  }
};
...
const issueCertificate = async (req, res, next) => {
  try {
    const enrollment = await evaluationService.issueCertificate(req.params.id);
