...
export default function CategoryPage() {
    ...
    const { courses, pagination, categoryInfo, loading } = useCoursesByCategory(
        slug,
        page,
        10
    );
    ...
    useEffect(() => {
        const searchCourses = async () => {
            try {
                ...
                const response = await fetch(
                    `${API_BASE}/courses/search?${params}`
                );
                const result = await response.json();
...
export const fetchCategories = async (): Promise<Category[]> => {
    const response = await fetch(`${API_BASE}/categories`);
    const result: ApiResponse<{ categories: Category[] }> =
        await response.json();
...
export const fetchPositions = async (): Promise<Position[]> => {
    const response = await fetch(`${API_BASE}/positions`);
    const result: ApiResponse<{ positions: Position[] }> =
        await response.json();
...
fetch(
    "https://class-crew.onrender.com/api/v1/public/courses/category/leadership?page=1&limit=10"
)
    .then((res) => res.json())
    .then((data) => {
...
fetch(
    "https://class-crew.onrender.com/api/v1/public/courses/search?category=dx&position=manager"
)
    .then((res) => res.json())
    .then((data) => {
